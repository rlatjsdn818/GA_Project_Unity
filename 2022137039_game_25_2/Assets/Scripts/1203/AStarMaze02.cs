...
public class AStarMaze02 : MonoBehaviour
{
    public int width = 21;
    ...
    public GameObject enemyPrefab;  // 인스펙터에 적 프리팹 연결
    ...
    void Start()
    {
        GenerateValidMap();  // 미로 생성 + 탈출 확인
        DrawMap();           // 시각화
        SpawnEnemies(5);     // 적 배치
        List<Vector2Int> path = RunAStar(map, startPos, goalPos); // 적 반영
        if (path == null)
            Debug.Log("경로 없음");
        else
        {
            ShowPath(path);  // 경로 시각화
            SpawnPlayer(path);  // 플레이어 이동
        }
    }
    ...
    List<Vector2Int> RunAStar(int[,] map, Vector2Int start, Vector2Int goal)
    {
        ...
        while (open.Count > 0)
        {
            ...
            foreach (var d in dirs)
            {
                ...
                int newG = gCost[cur.x, cur.y] + TileCost(nx, ny);
    ...
    int H(Vector2Int a, Vector2Int b)
    {
        ...
        {
            int dist = Mathf.Abs(a.x - enemy.x) + Mathf.Abs(a.y - enemy.y);
            if (dist == 0) enemyPenalty += 100;  // 겹치면 매우 큰 비용
            else enemyPenalty += 2 / dist;      // 가까울수록 조금씩 비용 추가
        }
    ...
    int enemyCount = 5; // 적 개수 예시

    int TileCost(int x, int y)
    {
        int tile = map[x, y];

        if (tile == 0) return int.MaxValue; // 벽 절대 통과 금지

        int cost = tile switch
        {
            1 => 1,
            2 => 3,
            3 => 5,
    ...
    void PlaceEnemies()
    {
        ...
        for (int i = 0; i < enemyCount; i++)
        {
            while (true)
            {
                ...
                if (map[x, y] == 1 && !enemies.Contains(new Vector2Int(x, y)) &&
    ...
    void SpawnEnemies(int count)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            while (map[x, y] == 0 || enemyPositions.Contains(new Vector2Int(x, y)) ||
                   (x == startPos.x && y == startPos.y) ||
                   (x == goalPos.x && y == goalPos.y));
